...
export default async function MessageDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!item || item.article_type !== 'Message') notFound();

  const allArticles = await newsService.getArticles();
  const relatedMessages = allArticles
    .filter((n) => n.id !== item.id && n.status === 'Published' && n.article_type === 'Message')
    .slice(0, 3);
        ...
        <section className="relative overflow-hidden bg-slate-50 py-14 lg:py-20">
          ...
          <div className="relative z-10 mx-auto max-w-7xl px-5 sm:px-8 lg:px-10">
            ...
            <div className="flex items-center gap-2 text-sm font-medium text-slate-500">
              ...
              <Link href="/#news" className="transition-colors hover:text-[#014BAA]">መልዕክት</Link>
              <ChevronRight className="size-4" />
