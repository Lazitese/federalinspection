...
export default function EditNewsPage({ params }: { params: { id: string } }) {
  ...
  const { register, handleSubmit, formState: { errors, isSubmitting }, watch, reset } = useForm<NewsFormValues>({
  ...
  useEffect(() => {
    newsService.getArticle(params.id).then(article => {
      if (article) {
        reset({
          title: article.title,
        ...
        const existing = article.images || (article.image ? [article.image] : []);
  ...
  const onSubmit = async (data: NewsFormValues) => {
    try {
      await newsService.updateArticle(params.id, { 
        ...data, 
        lang: data.language,
      ...
      router.push(`/dashboard/news/${params.id}`);
    } catch (error) {
      console.error(error);
    }
  };
    ...
    <DashboardLayout>
      <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-8 h-full max-w-4xl mx-auto pb-10">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4">
          <div>
            <Link href={`/dashboard/news/${params.id}`} className="text-xs font-semibold text-brand-blue uppercase tracking-widest hover:underline flex items-center gap-1 mb-2">
              <IconArrowLeft size={14} stroke={2} /> ወደ ዜና ይመለሱ
            </Link>
            <h1 className="text-3xl font-light text-text-primary tracking-tight">ዜና አስተካክል</h1>
            <p className="text-sm text-text-muted mt-1">ዜና #{params.id} እየተስተካከለ ነው።</p>
          </div>
          <div className="flex gap-3">
            <Link 
              href={`/dashboard/news/${params.id}`} 
            ...
            <button 
              ...
              className="flex items-center gap-2 bg-brand-yellow hover:bg-brand-yellow/90 text-[#3D352E] px-5 py-2.5 rounded-full text-sm font-semibold transition-colors shadow-sm disabled:opacity-50"
            >
        ...
        <div className="bg-surface-primary/30 rounded-[2rem] border border-border/20 p-8 backdrop-blur-md flex flex-col gap-6">
          ...
          <div className="grid grid-cols-2 gap-6">
            <div className="flex flex-col gap-2">
              ...
              <select 
                {...register("language")} 
                className="w-full bg-surface-primary border border-border/50 rounded-xl p-4 text-sm text-text-primary focus:outline-none focus:border-brand-yellow/50 transition-colors appearance-none cursor-pointer"
              >
          ...
          <div className="flex flex-col gap-3">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
              ...
              <button
                ...
                className="h-28 rounded-xl border-2 border-dashed border-border/50 bg-surface-primary/50 flex flex-col items-center justify-center gap-1.5 cursor-pointer hover:bg-surface-secondary/50 hover:border-brand-yellow/30 transition-all group"
              >
                <IconPhotoPlus size={24} stroke={1.5} className="text-text-muted group-hover:text-brand-yellow transition-colors" />
          ...
          <div className="flex flex-col gap-2">
            <label className="text-xs font-semibold text-text-secondary uppercase tracking-widest flex items-center gap-2">
              <IconVideo size={14} className="text-brand-yellow" />
