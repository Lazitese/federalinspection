...
export default function NewsDetailPage({ params }: { params: { id: string } }) {
  ...
  const relatedNews = item ? newsItems.filter((n) => n.id !== item.id).slice(0, 3) : [];
  if (!item) notFound();
          ...
          <div className="mx-auto max-w-7xl">
            <article className="overflow-hidden rounded-[2rem] bg-white shadow-[0_24px_64px_-24px_rgba(1,75,170,0.10)] ring-1 ring-slate-100/80">
              ...
              <div className="px-5 py-10 sm:px-8 lg:px-10 sm:py-14">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className="flex items-center gap-2 rounded-full bg-[#014BAA]/5 px-4 py-1.5">
                      ...
                      <time dateTime={item.date} className="text-xs font-bold text-[#014BAA]">{item.date}</time>
                    </div>
                ...
                <div className="mt-8 space-y-5 text-base leading-[1.85] text-slate-600 sm:text-lg">
                  <p>{item.description}</p>
                  <p>
                    ኮሚሽኑ በዚህ ጉዳይ ላይ ተጨማሪ መረጃዎችን በቀጣይ ቀናት ይፋ እንደሚያደርግ አስታውቋል። ለበለጠ መረጃ የኮሚሽኑን ይፋዊ ገፆች መከታተል ይችላሉ።
                  </p>
                </div>

                <div className="mt-8 flex flex-wrap items-center gap-2">
                  {["ዜና", "ኮሚሽን", "አስተዳደር"].map((tag) => (
