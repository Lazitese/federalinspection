...
export function FormTableRenderer({ schema, initialData = {}, onChange, isCompleted }: FormTableRendererProps) {
  ...
  return (
    <div className={`bg-bg-primary rounded-2xl border transition-all duration-200 overflow-hidden ${
      isCompleted 
      ...
      {/* Body / Form Inputs */}
      {isOpen && (
        <div className="p-5 pt-2 border-t border-border-light bg-bg-secondary/10 animate-in slide-in-from-top-2 duration-200">
          <div className="overflow-x-auto pb-2">
            <table className="w-full text-left min-w-[600px] md:min-w-full">
              <thead className="bg-bg-secondary rounded-xl text-xs font-semibold text-text-secondary uppercase tracking-wider">
                <tr>
              ...
              <tbody className="divide-y divide-border-light">
                {schema.columns.map((col, idx) => (
                  <tr key={col.key} className="hover:bg-bg-secondary/30 transition-colors">
                          ...
                          {col.subKeys.map(subKey => (
                            <div key={subKey} className="flex-1 space-y-1.5">
                              ...
                              <div className="relative group">
                                <span className="absolute -top-2.5 left-2 bg-bg-primary px-1 text-[10px] text-text-muted font-medium hidden md:block z-10">
                                  {subKey}
                                </span>
                                <input
                                  type="number"
                                  ...
                                  className="w-full px-3 py-2.5 bg-bg-primary border border-border-medium rounded-lg text-sm text-text-primary focus:outline-none focus:border-brand-blue focus:ring-1 focus:ring-brand-blue transition-all"
                                  placeholder="0"
                                />
                              </div>
                            </div>
